package GenerateExamDocsPorject;
...
/**
 * Master Seating Plans class
 * 
 * @author Zhiheng (David) Li
 * March 16, 2016
 */
public class MasterSeatingPlans extends GenDocument {
	...
	public MasterSeatingPlans() {
		super();
	}	
	public MasterSeatingPlans(String courseName, int courseID, char courseSection) {
	...
	public boolean generateSheet() {
    	...
    	try {
    		...
    		String sql = "Select courseName, courseID, section, month, day, year, weekday, time, building, room, row, instructor, duration from ExamData"
    				+ " where "  + " month = '" + getMonth() + "' AND day =" + getDay() + " AND time =" + getTime() +";";
    ...
    public boolean generateTable(ResultSet set) {
        try {      	
            String filename = "C:/GenerateDocs/GenerateMasterSeatingPlans.xls" ;
            ...
            rowhead.createCell(4).setCellValue("Master");            
            ...
            row2.createCell(5).setCellValue("Building");
            row2.createCell(6).setCellValue("Room/Row");
            row2.createCell(7).setCellValue("Instructor");  
            int i = 3;
            while(set.next()) {
	    		...
	    		this.duration = Float.parseFloat(set.getString(13));
                ...
                row.createCell(4).setCellValue(this.duration);
                row.createCell(5).setCellValue(getBuilding());
                row.createCell(6).setCellValue(this.room == -1? getRowNum() : getRoom());
                row.createCell(7).setCellValue(getInstructor());
                i++;
            }
            if(i == 3) {
    			workbook.close();
